...
- 文章与笔记的文件名决定访问地址（`/posts/<文件名>/`、`/notes/<文件名>/`），项目为 `/projects/<文件名>/`；**已发布条目的文件名视为冻结**——编辑表单里虽可修改，但改名等于换 URL，外部链接会失效，确需修改按第 5 节回滚/迁移流程处理；
- 文件名约定：小写 ASCII 字母、数字、连字符（指南 12）；
- 配置使用 filename 对象形式（`{template, field: true}`，IMPL-043）：创建与编辑表单都有独立的「文件名」输入框，**标题与路由 slug 分开决定**——标题可以直接写中文，文件名单独填英文 slug（如 `astro-from-scratch-4`），不再依赖标题 slugify；
- 笔记可选在文件名前加日期前缀（如 `2026-09-10-xxx`），与既有内容保持一致；项目固定使用语义化短名（如 `cove-site`）；
- 分类内容区没有文件名输入框（`field: false`）：文件名始终由标识（key）字段派生，两处永远一致，不存在改动不同步的问题。
...
1. **date 字段输出**：~~保存后检查 frontmatter 是否为带引号的 `'YYYY-MM-DD'` 字符串~~ **已实测（IMPL-042）**：CMS 保存的日期不加引号，YAML 解析为 Date 对象；schema 已用 preprocess 兼容（Date → `YYYY-MM-DD` 字符串），本地手写带引号字符串仍正常，此项关闭；
...
5. **filename 字段**：配置已改为对象形式（IMPL-043，官方 filename 文档核实：`{template, field: true}` 在创建与编辑表单均显示文件名输入框）。首次使用时实测确认：创建条目时「文件名」输入框出现、中文标题不再影响文件名、编辑已发布条目时输入框可见（改 slug 会换 URL，谨慎操作）；
