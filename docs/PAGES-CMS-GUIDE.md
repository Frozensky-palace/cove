...
3. 打开 `cove` 仓库，CMS 按 `.pages.yml` 渲染出三个内容区：文章、笔记、项目。
...
| 分类 | `category` 枚举 | 下拉固定五项，与 `src/data/taxonomy.ts` 同步 |
...
- **相关文章**：CMS 中手填 ID，无关联选择器（reference 字段写入路径而非 ID，与 schema 不符，未采用）；
